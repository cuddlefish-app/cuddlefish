    ...
    try {
        ...
        if (e instanceof vscode.FileSystemError) {
            if (e.code === "FileNotFound" || e.code === "FileNotADirectory") {
                ...
                console.log(`fs.stat returned a FileSystemError with an unexpected code: ${e.code}`);
                throw e;
            }
        }
        else {
...
// return undefined.
async function gitRepoPath(path) {
...
async function activate(context) {
    ...
    const credentials = new credentials_1.Credentials(context);
    ...
    //   })
    // );
    context.subscriptions.push(vscode.commands.registerCommand("cuddlefish-comments.helloWorld", async () => {
        console.log(vscode.workspace.textDocuments.map((doc) => doc.fileName));
        let foo = vscode.workspace.textDocuments[0];
        const octokit = await (0, credentials_1.getOctokitModal)(credentials);
    ...
    // comment controller
    let commentController = vscode.comments.createCommentController("cuddlefish-comments", "Where does this even go?");
    commentController.commentingRangeProvider = {
        provideCommentingRanges: async (document, _token) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cuddlefish-comments.startThread", async (reply) => {
        ...
        if (gitRepoRoot === undefined) {
            throw new Error("tried to startThread in a file that does not seems to be in a git repo");
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("cuddlefish-comments.commentOnThread", (reply) => {
        // TODO
